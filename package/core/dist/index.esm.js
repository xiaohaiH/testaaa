...
const IS_COMPOSITION_VERSION = version.slice(0, 3) === '2.7';
/** 容器注入的 key */
...
function useWrapper(props) {
  ...
  provide(provideKey, wrapperInstance);
...
function useDisplay(props, option) {
  ...
  let listeners = [watch(() => props.query, cb, {
    immediate: true,
    deep: true
  }), watch(() => [props.disabled, props.hide], (val, val2) => {
    if (val[0] !== val2[0]) {
...
function usePlain(props) {
  /** 容器注入值 */
  const wrapper = inject(provideKey);
  /** 初始值(重置时回填的值) */
  ...
  unwatchs.push(watch(() => props.field, (val, oldVal) => {
    val !== oldVal && wrapper?.removeUnreferencedField(oldVal);
    option.updateWrapperQuery();
  }));
  // 实时值发生变化时触发更新 - 共享同一个字段
  unwatchs.push(watch(() => [props.fields || props.field, props.fields ? props.fields.map(k => props.query[k]).filter(Boolean) : props.query[props.field]],
...
function useTree(props) {
  ...
  watch(sourceIsInit, val => val && initCheckedInfo(), {
    immediate: true
  });
  /** 设置初次选中的值以及初始值 */
  ...
  // 提交字段发生改变时, 删除原有字段并更新最新值
  unwatchs.push(watch(() => props.fields || [props.field], (val, oldVal) => {
    val.toString() !== oldVal.toString() && wrapper && oldVal.forEach(o => val.includes(o) || wrapper.removeUnreferencedField(o));
    option.updateWrapperQuery();
  }));
  // 实时值发生变化时触发更新 - 共享同一个字段
  unwatchs.push(watch(() => [props.fields?.toString() || props.field, props.fields?.map(v => props.query[v]).filter(Boolean) || props.query[props.field]], ([_field, val], [__field, oldVal]) => {
    // 仅在值发生变化时同步
  ...
  // 回填值发生变化时触发更新
  unwatchs.push(watch(() => props.fields?.length ? props.fields.reduce((p, k) => {
